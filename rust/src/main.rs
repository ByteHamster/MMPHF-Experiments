...
fn bench_int64(dataset: &str, keys: &[u64], num_queries: usize) -> Result<()> {
    ...
    let func: Unaligned<LcpMmphfInt<u64>> = LcpMmphfInt::try_new_with_builder(
        FromSlice::new(keys),
        keys.len(),
...
fn bench_strings(dataset: &str, keys: &[&str], num_queries: usize) -> Result<()> {
    ...
    let start = Instant::now();
    let mut pl = ProgressLogger::default();
    let func: Unaligned<LcpMmphfStr> = LcpMmphfStr::try_new_with_builder(
        FromSlice::new(keys),
        keys.len(),
        VBuilder::default().max_num_threads(1),
        &mut pl,
...
fn bench_int64_lcp2(dataset: &str, keys: &[u64], num_queries: usize) -> Result<()> {
    ...
    let start = Instant::now();
    let mut pl = ProgressLogger::default();
    let func: Unaligned<Lcp2MmphfInt<u64>> = Lcp2MmphfInt::try_new_with_builder(
        FromSlice::new(keys),
        keys.len(),
        VBuilder::default().max_num_threads(1),
        &mut pl,
...
fn bench_int32(dataset: &str, keys: &[u32], num_queries: usize) -> Result<()> {
    ...
    let start = Instant::now();
    let mut pl = ProgressLogger::default();
    let func: Unaligned<LcpMmphfInt<u32>> = LcpMmphfInt::try_new_with_builder(
        FromSlice::new(keys),
        keys.len(),
        VBuilder::default().max_num_threads(1),
        &mut pl,
...
fn bench_int32_lcp2(dataset: &str, keys: &[u32], num_queries: usize) -> Result<()> {
    ...
    let start = Instant::now();
    let mut pl = ProgressLogger::default();
    let func: Unaligned<Lcp2MmphfInt<u32>> = Lcp2MmphfInt::try_new_with_builder(
        FromSlice::new(keys),
        keys.len(),
        VBuilder::default().max_num_threads(1),
        &mut pl,
...
fn bench_strings_lcp2(dataset: &str, keys: &[&str], num_queries: usize) -> Result<()> {
    ...
    let start = Instant::now();
    let mut pl = ProgressLogger::default();
    let func: Unaligned<Lcp2MmphfStr> = Lcp2MmphfStr::try_new_with_builder(
        FromSlice::new(keys),
        keys.len(),
        VBuilder::default().max_num_threads(1),
        &mut pl,
