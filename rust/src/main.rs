...
fn bench_int64(dataset: &str, keys: &[u64], num_queries: usize) -> Result<()> {
    ...
    let func: LcpMmphfInt<u64, [u64; 2], FuseLge3Shards> =
        LcpMmphfInt::new(FromSlice::new(keys), n, &mut pl)?;
...
fn bench_strings(dataset: &str, keys: &[String], num_queries: usize) -> Result<()> {
    ...
    let start = Instant::now();
    let mut pl = ProgressLogger::default();
    let func: LcpMmphfStr<[u64; 2], FuseLge3Shards> =
        LcpMmphfStr::new(lender, n, &mut pl)?;
    let construction_ms = start.elapsed().as_millis() as u64;
